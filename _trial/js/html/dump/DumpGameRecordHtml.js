class DumpGameRecordHtml {
   ...
   */
  update(gameRecords, noticeIds, rowClickListener) {
    // テーブル更新
    this._clearTable();
    this._updateTable(gameRecords, noticeIds, rowClickListener);
   ...
   * @param {Object} rowClickListener
   */
  _updateTable(gameRecords, noticeIds, rowClickListener) {
  ...
  _getGameRecordFromRow(html_tr) {
    ...
    gameRecord.fighter   = this._html_td_fighter(html_tr).data("sort-value");
    gameRecord.isVip     = this._html_td_isVip(html_tr).data("sort-value");
    gameRecord.isDeleted = this._html_td_isDeleted(html_tr).data("sort-value");
    return gameRecord;
  }
