class RecordAnalyzer {
  ...
  constructor(gameRecords) {
    ...
    this._inconsistencyGameRecordIds = [];
   ...
   * @return {Integer}
   */
  get inconsistencyGameRecordIds() {
    return this._inconsistencyGameRecordIds;
  }
  ...
  _updateFighterRecords(gameRecord) {
    var fighter = gameRecord.fighter;
    var stock   = gameRecord.stock;
    this._fighterRecords.add(fighter, stock);
  }
  ...
  _updateInconsistencyGameRecordIds(gameRecord) {
    ...
    if ((rate < this._gameRecord_prev.rate) && (stock > 0)) {
      this._inconsistencyGameRecordIds.push(id);
      return;
    }
