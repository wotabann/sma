class DumpHistoryChartHtml {
  ...
  get _html_section()  { return $("#dump-history-chart"); }
  ...
  get _html_chart()   { return $(this._html_body.find(".dump-history-chart")); }

  get _html_canvasLatest() { return $(this._html_chart.find(".canvas-latest")); }
  get _html_canvasAll()    { return $(this._html_chart.find(".canvas-all")); }
  get _html_canvasDaily()  { return $(this._html_chart.find(".canvas-daily")); }
  ...
  update(gameRecords, dailyRecords) {
    ...
    var dailyData  = this._createDailyData(dailyRecords, 20);
  ...
  _createLatestData(gameRecords, plotCount) {
    ...
    for (let i = headIndex; i < length; i++) {
      if (gameRecords.index(i).isDisabled == 0) {
  ...
  _createDailyData(dailyRecords, plotCount) {
    ...
    var labels = [];
    for (let i = headIndex; i < length; i++) {
      rates.push(dailyRecords.index(i).rate);
