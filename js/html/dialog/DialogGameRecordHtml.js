class DialogGameRecordHtml {
  ...
  get _html_section()    { return $("#dialog-game-record"); }
  ...
  set head(text) { this._html_head.text(text); }
   ...
   * @note ファイターでフィルタする
   */
  filterByFighter(fighterId) {
    this._filter("fighter-id", fighterId);
  }
  ...
  _setGameRecordToRow(html_tr, gameRecord) {
    var fighterName = Fighter.idToName[gameRecord.fighterId];
